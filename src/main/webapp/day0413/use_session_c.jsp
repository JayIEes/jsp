<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"
   	info="이전 페이지에서 사용한 값을 사용할 페이지 - 세션에서 얻는다."
    %>
<!DOCTYPE html>
<html>
<head>
...
<body>
<%
//이전 페이지에서 사용된 값을 이동한 페이지에서는 사용할 수 없다.(값이 유지가 안됨.) - 세션을 사용하여 새션애 저장된 값을 얻는다.
String name=(String)session.getAttribute("user_name");
if(name==null){
	//세션에서 얻은 값이 없을 때 
	//1. 직접 요청했을 때(개발자가 의도한 요청의 순서가 아닐 때) 
	//2.세션이 죽었을 때 (개발자가 의도한 요청의 순서였지만 데이터의 유지시간을 초과했을 때)
	response.sendRedirect("use_session_a.jsp");
	return;
}
%>
<%=name %>님 안녕하세요?<br>
이동한 작업 페이지에서 이전 페이지의 값을 사용할 수 있다.<br>
